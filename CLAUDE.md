...
**완료 카드 시각 표시**: **배경 옅은 회색 `#DFE2E6` + 얇은 짙은 녹색 박스 테두리 `#15803D`** + 기존 `✅ 완료` 상태 배지 유지.
...
**적용처**:
- PC 허브 `_hubRenderGroup`: `g.jobs.slice().sort(window._jobDoneSort)` (전 카테고리 공통). 완료 서브카드 `.hub-sj.done{background:#DFE2E6;border-color:#15803D}`(app.css).
- PC AS관리 `renderAsMgmt`: `view.sort(window._jobDoneSort)` + 완료 행 `background:#DFE2E6` + 녹색 좌측 accent(`box-shadow:inset 2px 0 0 #15803D`).
- 모바일 5개 SPA `renderEntry`: `.sort(window._jobDoneSort)` + `.card.done{background:#DFE2E6;border:1px solid #15803D}`.
...
| **완료 표시** | **배경 옅은 회색 `#DFE2E6` + 얇은 짙은 녹색 박스 테두리 `#15803D`** + `✅ 완료` 배지 (PC `.hub-sj.done`·AS관리 행·모바일 `.card.done`). |
...
**검색 규칙** (`_scoreStore`):
- 질의를 **구분자(공백 + `/ , · ; |`)로 토큰 분리** → 각 토큰을 **상호(name/storeName) · 주소(address/addr) · 사업자번호(숫자) · 대표(ceo) · 거래처코드(code) · aliases** 에서 매칭. (`-`·`.` 은 분리 안 함 — 사업자번호 보존.)
- **비연속·역순·주소먼저 매칭**: "테스트마트 노원", "노원 오케이마트", "서울/노원/상계 오케이마트"(주소 토큰이 앞·상호가 뒤) 모두 매칭됨. **상호가 앞에 올 필요 없음.**
...
// 완료 카드/행: background:#DFE2E6 (옅은 회색) + 얇은 짙은 녹색 테두리 #15803D + ✅완료 배지
```
...
- `ns_jobs_snap` localStorage 키에 각 job 의 **짧은 해시** 저장 (`_jobHashForMtime` = `_fastHash(JSON.stringify(job−updatedAt))`)
  ...
  - 🔴 **`ns_jobs`/`ns_stores` 를 `localStorage.setItem` 으로 직접(raw) 쓰지 말 것 — 반드시 `_safeSetItem` 경유** (2026-06-12): `saveJobs/saveStores` 만 고치고 **`_forceResyncFromCloud`(새로고침 강제 재동기화)·수동 resync·`m/index.html`·`m/diag.html` 의 raw 쓰기를 놓쳐** QuotaExceededError 재발했음. 점검: `grep -rnE "localStorage\.setItem\('ns_(jobs|stores)'" m-core.js m/*.html` → `_safeSetItem` 미경유 0 이어야 함.
  - 🔴🔴 **진짜 근본원인 = `ns_stores` 단독 5MB 초과** (2026-06-12): 라이브 `/api/stores` = **~5MB** (매장 2807개). 그 중 **`contacts` 가 ~3.5MB**(2026-06-11 연락처 적재 백필 누적). UTF-16 환산 시 ns_stores 단독 ~10MB > 한도 5MB → `_safeSetItem`/eviction 으로도 저장 불가(값 자체가 큼). ⚠ 진단 시 **로컬 `_s.json`(오래된 사본) 말고 라이브 `/api/stores` 실측**: `curl -s .../api/stores -o /dev/null -w '%{size_download}'`.
  - ✅ **해법 = 모바일 lean 저장** (`m-core` `global._leanStores`): 모바일이 **화면에서 안 읽는** 무거운 매장 배열(`contacts`/`equipment`/`changeLog`/`storeMemos`/`memos`)을 localStorage 저장 직전 제거 → ns_stores 4.3MB→**0.7MB**. 클라우드/PC 는 전체 보존(모바일 `saveStores` 는 cloud push 안 함, 머지 `additive-by-id` 라 안전). 적용처: `saveStores`(→`syncStoresFromCloud` 자동 커버)·`_forceResyncFromCloud`·`m/index.html`. **모바일이 해당 필드 UI 를 도입하면 `_LEAN_STORE_DROP` 에서 빼고 다른 저장전략(IndexedDB/압축) 검토.** 그래도 터지면 iOS **시크릿(비공개) 탭** 의심(quota≈0).
  - 🔴 **레거시 잔존 함정** (2026-06-12): lean 코드를 배포해도, 기존 기기에 이미 저장된 **옛 full `ns_stores`(~5MB)가 안 지워져** "저장공간 부족" 토스트가 계속 떴음. 원인 = `syncStoresFromCloud` 가 **ETag 304(변경없음)면 `ns_stores` 를 안 건드림** → 옛 bloat 영구 잔존. 해결: ① m-core 로드 시 **1회 즉시 재압축**(`ns_stores.length>1.1MB` 면 `_leanStores` 후 저장) ② `_safeSetItem` 실패 시 **2차 회복으로 `ns_stores` 재압축 후 재시도**. 즉 lean 도입 시 **신규 쓰기뿐 아니라 기존 저장분 재압축 경로도 필수**.
  - ⚠ **분리 부채(follow-up)**: `contacts` 가 무한 누적(백필 over-ingest)이라 PC localStorage(전체 저장)도 장기적으로 위험. source 정리(연락처 dedup/상한) 별도 과제 필요.
