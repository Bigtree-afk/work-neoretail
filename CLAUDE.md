...
| 임계 | 액션 |
|---|---|
| **신규 파일 작성** | 1,000줄 초과 예상되면 **처음부터 모듈 분할** 설계 |
| **기존 파일 신규 코드 추가** | 추가 후 1,000줄 초과면 분할 검토 — 새 함수는 별도 파일로 추출 우선 |
| **이미 1,000줄 초과인 파일** | 신규 코드 추가는 가능하되 **분할 부채 인식** + 관련 함수를 공유 모듈로 빼낼 기회 적극 탐색 |
| **예외** | 자동 생성 데이터, vendor 번들, HTML 단일 SPA 의 marker (단 JS 로직은 외부 파일로 분리) |

**현재 임계 초과 파일** (분할 부채):
- `index.html` (~22,000줄) — PC 단일 SPA
- `m-core.js` (~1,000줄) — 모바일 공유 모듈
- `m/newjob/index.html` (~1,700줄) — 모바일 신규 SPA
