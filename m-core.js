...
(function(global) {
  ...
  // ── STOCKTAKE — 클라우드 정본(cloud-native, 레코드 단위) ─────────
  //   getStocktakes: 로컬 캐시(동기 읽기). saveStocktakes: 캐시 저장 + 변경분만 클라우드 push.
  //   syncStocktakeFromCloud: pending flush → GET → 로컬-only 백필 업로드 → 캐시=클라우드(정본).
  ...
  function _stReadPending() { try { return JSON.parse(localStorage.getItem('ns_stocktake_pending') || '{"recs":{},"dels":[]}'); } catch { return { recs:{}, dels:[] }; } }
  function _stWritePending(p) { try { localStorage.setItem('ns_stocktake_pending', JSON.stringify(p)); } catch(_){} }
  function _stQueue(records, deletedIds) {
    const p = _stReadPending();
  ...
  async function _stFlushPending() {
    const p = _stReadPending(); const recs = Object.values(p.recs || {}); const dels = p.dels || [];
    ...
    try {
      ...
      _stWritePending({ recs:{}, dels:[] }); return true;
  ...
  function saveStocktakes(arr) {
    ...
    try {
      ...
      if (changed.length || deletedIds.length) { _stQueue(changed, deletedIds); _stFlushPending(); }
    } catch(_){}
  }
  async function syncStocktakeFromCloud() {
    try {
      await _stFlushPending();   // ① 밀린 쓰기 먼저 반영
  ...
  global.syncStocktakeFromCloud = syncStocktakeFromCloud;
